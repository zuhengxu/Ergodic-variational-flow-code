...
for i in 1:30
    uha_dat = JLD.load("result/uha_heavy_reg_" * string(i) *".jld")
    if i in [1:5;]
...
# hline!([0.15], linestyle=:dash, lw = 5, label = "NEO", color = colours[end-1])
...
p = plot(p1, p2, layout = (1, 2), title = "t Regression")
savefig(p, "figure/treg_mean_est.png")
...
p = plot(p1, p2, layout = (1, 2), title = "t Regression")
savefig(p, "figure/treg_var_est.png")