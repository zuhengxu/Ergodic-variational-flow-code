...
for i in 1:30
    uha_dat = JLD.load("result/uha_lin_reg_heavy_" * string(i) *".jld")
    if i in [1:5;]
...
hline!([396.97], linestyle=:dash, lw = 5, label = "NEO", color = colours[end-1])
...
p = plot(p1, p2, layout = (1, 2), title = "Linear Regression (heavy)")
savefig(p, "figure/lrh_mean_est.png")
...
p = plot(p1, p2, layout = (1, 2), title = "Linear Regression (heavy)")
savefig(p, "figure/lrh_var_est.png")