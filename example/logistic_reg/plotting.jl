...
NF_RealNVP5 = JLD.load("result/RealNVP5_run.jld2")
NF_RealNVP8 = JLD.load("result/RealNVP8_run.jld2")
Planar = JLD.load("result/Planar_run.jld2")
Radial = JLD.load("result/Radial_run.jld2")
...
# real nvp 8 is better because it has no nan
...
for i in 1:30
    uha_dat = JLD.load("result/uha_log_reg_" * string(i) *".jld")
    if i in [1:5;]
...
# el_nf = NF["elbo"]
eps = ELBO["eps"]
Els = Matrix(ELBO["elbos"][:,1:end-1][2,:]')
Ns = ELBO["Ns"][1:end-1]
...
hline!([0.72], linestyle=:dash, lw = 5, label = "NEO", color = colours[end-1])
...
NF = JLD2.load("result/RealNVP8.jld") 
...
NF_nvp = JLD.load("result/RealNVP8.jld2")
