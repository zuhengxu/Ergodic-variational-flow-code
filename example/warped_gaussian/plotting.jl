...
p_tune = plot(eps, Els, lw = 3, label = "", xlabel = "ϵ", ylabel = "ELBO", title = "Warped Gaussian", xtickfontsize=18, ytickfontsize=18, guidefontsize=18, legendfontsize=18, titlefontsize = 18, xrotation = 20, margin=5Plots.mm) 
savefig(p_tune, "figure/warped_gaussian_tuning.png")
...
p_elbo = plot(reduce(vcat, [[0], Ns]), Els',lw = 3, labels = Labels, legend = :outertopright, ylabel = "ELBO", xlabel = "#Refreshment", title = "Warped Gaussian", xtickfontsize=18, ytickfontsize=18, guidefontsize=18, legendfontsize=18, titlefontsize = 18, xrotation = 20, margin=5Plots.mm)
savefig(p_elbo, "figure/warped_gaussian_elbo.png")
...
# 1	0.2	10	0.11376560884647742	10	0.49842748565146494	24.484531416	0.0
# 2	0.2	10	0.10036869476679755	10	0.35990597949010605	12.627562003	0.0
# 3	0.2	10	0.09464560485033369	10	0.5500121211920491	15.832668975	0.0

ksd_neo = mean([0.49842748565146494, 0.35990597949010605, 0.5500121211920491])
...
plot(lap["Ns"], vec(median(lap["fwd_err"], dims=2)), ribbon = get_percentiles(lap["fwd_err"]), lw = 3, label = "Lap Fwd", xlabel = "#refreshments", ylabel = "error", title = "Warped Gaussian", xtickfontsize=18, ytickfontsize=18, guidefontsize=18, legendfontsize=18, titlefontsize = 18, xrotation = 45, margin=5Plots.mm, legend=:outertopright) 
...
savefig("figure/warped_gaussian_stability.png")
...
savefig(p_elbo, "figure/warp_elbo_nf.png")
...
savefig(p, "figure/nf_warp_scatter.png")
...
boxplot!(["NUTS"], ess_time_nuts[1:9], label = "NUTS", color = colours[2])
...
p2 = plot(iters, vec(median(m_nuts[:, 2, :]'; dims =1)), ribbon = get_percentiles(m_nuts[:, 2, :]), lw = 3,label = "NUTS")
    plot!(iters, vec(median(m_hmc[:, 2, :]'; dims =1)), ribbon = get_percentiles(m_hmc[:, 2, :]), lw = 3,label = "HMC", legend = :bottomright)
    ...
    hline!([0.0],  linestyle=:dash, lw = 2,color = :black, label = "Mean")
p = plot(p1, p2, layout = (1, 2), title = "Warped Gauss")
savefig(p, "figure/warp_mean_est.png")
...
p1 = plot(iters, vec(median(v_nuts[:, 1, :]'; dims =1)), ribbon = get_percentiles(v_nuts[:, 1, :]), lw = 3,label = "NUTS")
    ...
    hline!([sqrt(0.5)],  linestyle=:dash, lw = 2,color = :black, label = "SD")
...
p2 = plot(iters, vec(median(v_nuts[:, 2, :]'; dims =1)), ribbon = get_percentiles(v_nuts[:, 2, :]), lw = 3,label = "NUTS")
    ...
    hline!([sqrt(0.5)],  linestyle=:dash, lw = 2,color = :black, label = "SD")
p = plot(p1, p2, layout = (1, 2), title = "Warped Gauss")
savefig(p, "figure/warp_var_est.png")
###########33
# further NF results
############
...
hline!([-0.38], linestyle= :dash,  label= "MixFlow", lw =3, legend = :topright, legendfontsize = 15)
