...
ϵ0 = 0.0001*ones(2)
verb = 1000
ref, inv_ref= ErgFlow.pseudo_refresh, ErgFlow.inv_refresh
opt_args = (elbo_size = 1, niters = niters, learn_init = false, verbose_freq = verb, optimizer = Flux.ADAM(1e-3))
# μ0, D0 = μ, D
μ0, D0 = zeros(2), ones(2)
D_nuts = nuts(μ, 0.7, logp, ∇logp, 1000, 10000)
ksd_nuts = ksd(D_nuts, ∇logp; bw = bw) #0.213862

normal_mom = (randn, ErgFlow.lpdf_normal, ErgFlow.∇lpdf_normal, ErgFlow.cdf_normal, ErgFlow.invcdf_normal, ErgFlow.pdf_normal) 
# # rough compare 
# n_lfrg = 50
...
JLD.save("result/metric_norm_ef$(n_lfrg)_$(n_mcmc).jld", "ELBO", el, "KSD", ksd_ef, "ksd_nuts",ksd_nuts , "ps_trace", PS)

o_his = HVI.HIS(d, n_lfrg, n_mcmc, logp, ∇logp, randn, logq, 
        randn, ErgFlow.lpdf_normal, ErgFlow.∇lpdf_normal)
...
JLD.save("result/metric_norm_his$(n_lfrg)_$(n_mcmc).jld", "ELBO", el_his, "KSD", ksd_his, "ps_trace", PS_his)
...
JLD.save("result/metric_norm_uha$(n_lfrg)_$(n_mcmc).jld", "ELBO", el_uha, "KSD", ksd_uha, "ps_trace", PS_uha)

# plot([ksd_ef, ksd_his, ksd_uha], lw = 3, alpha = 0.6 ,label = ["EF" "HIS" "UHA"])
# hline!([ksd_nuts], linestype=:dash, lw = 2)


ef = JLD.load("result/metric_norm_ef$(n_lfrg)_$(n_mcmc).jld")
his = JLD.load("result/metric_norm_his$(n_lfrg)_$(n_mcmc).jld")
uha = JLD.load("result/metric_norm_uha$(n_lfrg)_$(n_mcmc).jld")
p = plot([ef["KSD"], his["KSD"], uha["KSD"]], lw = 3, alpha = 0.6 ,label = ["EF" "HIS" "UHA"])
hline!([ef["ksd_nuts"]], linestype=:dash, lw = 2)
savefig(p, joinpath("figure/","ksd_norm$(n_lfrg)_$(n_mcmc).png"))
...
savefig(p_el, joinpath("figure/","elbo_norm$(n_lfrg)_$(n_mcmc).png"))
